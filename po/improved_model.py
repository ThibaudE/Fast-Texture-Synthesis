...
class ProgressiveSynTex(SynTexModelDesc):
    ...
    def build_graph(self, pre_image_input, image_target):
        ...
        def viz(name, images):
            ...
            tf.compat.v1.summary.image(name, im, max_outputs=10, collections=["image_summaries"])
        ...
        with tf.compat.v1.variable_scope("syn"):
    ...
    def optimizer(self):
        lr_var = tf.compat.v1.get_variable("learning_rate", initializer=self._lr, trainable=False)
        add_tensor_summary(lr_var, ['scalar'], main_tower_only=True)
        return tf.compat.v1.train.AdamOptimizer(lr_var, beta1=self._beta1, beta2=self._beta2, epsilon=1e-3)
